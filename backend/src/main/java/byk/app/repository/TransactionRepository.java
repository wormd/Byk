...
//    @Query("select t from Transaction t where ((?1 is null or t.created >= ?1) and (?2 is null or t.created <= ?2))")
    ...
    Page<Transaction> findByCreatedAndDates(LocalDate after, LocalDate before, Pageable pageable);

    @Query("select t from Transaction t where t.date >= ?1 and t.date <= ?2")
    Page<Transaction> findByDateAndDates(LocalDate after, LocalDate before, Pageable pageable);
